...
VAR
	...
	begin
		...
		writeln('Input data');
		writeln('=================================================================');
		write('Nama Korban                                   : ');readln(A[i].namalpr);
		write('Masukan No.KTP Korban                         : ');readln(A[i].noktp);
		write('Nama Pelaku                                   : ');readln(A[i].namaplk);
		write('Jenis Kejahatan (Pembunuhan/Korupsi/Penipuan) : ');readln(A[i].jenis);
        repeat
	...
	begin
		...
		repeat
			...
			for i:=0 to length(A)-1 do begin
				writeln('No Kasus        : ',i+1);read;
				writeln('No KTP Korban   : ',A[i].noktp);read;
				writeln('Nama Korban     : ',A[i].namalpr);read;
				writeln('Nama Pelaku     : ',A[i].namaplk);read;
				writeln('Jenis Kejahatan : ',A[i].jenis);read;
	...
	procedure menusidang(h : shortstring);
        begin
	        write('Masukan No Kasus : ');
                readln(N);
                {writeln(A.[N]);
                readln;}
                if A[N].jenis='pembunuhan' then begin
                        writeln('Hukumanmu Seumur Hidup, karena nyawa di bayar nyawa');
                        readln;
                end;
                if A[N].jenis='korupsi' then begin
                        writeln('Hukumanmu Denda 200M dan penjara 20 tahun');
                        readln;
                end;
                if A[N].jenis='penipuan' then begin
	...
	begin
		...
		while not EOF(f) do begin
			...
			for i:=0 to length(A)-1 do begin
				...
				if h=' ' then begin
					writeln('Anda Belum Melakukan Sidang');
					readln;
				end;
			writeln('=================================================================');
				readln;
